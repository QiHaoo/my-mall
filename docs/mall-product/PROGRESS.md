...
| 2026-06-28 | 分类与品牌管理接口实现同步 v1.1 | `b67f0ee` | 按最新设计文档同步后端实现：分类字段统一 catId、show_status 业务删除、批量删除增加 SPU 引用检查与关联表冗余名同步；品牌表单剥离 categoryIds、新增批量删除与独立关联管理接口；ResultCode 补充 53006-53008；CategoryServiceTest/BrandServiceTest/CategoryControllerTest/BrandControllerTest 全绿（28 个用例通过）。 |
