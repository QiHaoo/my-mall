...
| 2026-06-28 | 品牌管理接口设计重构 v1.1 | `ee9e06c` | 配合前端「关联分类独立弹窗」设计，将关联分类管理从品牌新增/修改接口中剥离，改为独立接口（查询/新增/移除）；新增批量删除品牌接口；DTO/VO 扩展至 8 个；错误码补充 53006-53008；冗余字段同步改由 `CategoryBrandRelationService` 内部方法 `updateBrandName`/`updateCategoryName` 承担。**仅文档调整，代码实现待同步**（BrandServiceImpl 需移除 categoryIds 全量覆盖逻辑、新增 CategoryBrandRelationService/Controller 等）。 |
| 2026-06-28 | 分类与品牌管理接口实现同步 v1.1 | `b67f0ee` | 按最新设计文档同步后端实现：分类字段统一 id、show_status 业务删除、批量删除增加 SPU 引用检查与关联表冗余名同步；品牌表单剥离 categoryIds、新增批量删除与独立关联管理接口；ResultCode 补充 53006-53008；CategoryServiceTest/BrandServiceTest/CategoryControllerTest/BrandControllerTest 全绿（28 个用例通过）。 |
| 2026-07-01 | 商品分类字段命名规范化 | — | 全链路统一字段命名，消除历史遗留不一致：`catId`→`id`（主键对齐 BaseEntity）、`parentCid`→`parentId`、`catLevel`→`level`、`catelogId`→`categoryId`（修正拼写）、`catalogId`→`categoryId`（统一 catalog→category）、`catelogName`→`categoryName`、`updateCatelogName`→`updateCategoryName`。涉及 6 张表 SQL（字段+索引）+ 6 实体 + 6 DTO/VO + Service/Controller/Mapper + 3 测试类（61 用例全绿）+ 前端 5 组件 + 7 篇模块文档 + 2 篇规范文档。 |
...
- **~~品牌-分类关联同步用 diff 而非"全删再插"~~**（⚠️ v1.1 已废弃）：v1.1 设计改为关联分类独立单条增删，不再有"全量覆盖/diff"逻辑。生产彻底解法建议唯一索引改为 `uk_brand_category(brand_id, category_id, is_deleted)`。
